...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["import_csv"])) {
    if (is_uploaded_file($_FILES['csv_file']['tmp_name'])) {
        ...
        while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
            ...
            $kd_beasiswa = $connection->real_escape_string($data[0]);
            $kd_kriteria = $connection->real_escape_string($data[1]);
            $nim         = $connection->real_escape_string($data[2]);
            $nilai       = $connection->real_escape_string($data[3]);

            // Insert ke database
            $insert = $connection->query("
                INSERT INTO nilai (kd_nilai, kd_beasiswa, kd_kriteria, nim, nilai) 
                VALUES (NULL, '$kd_beasiswa', '$kd_kriteria', '$nim', '$nilai')
            ");
        ...
        fclose($handle);
        echo alert("Import selesai: $success data berhasil, $fail gagal.", "?page=nilai");
    } else {
        echo alert("Gagal mengunggah file CSV!", "?page=nilai");
    }
}
	    ...
	    <div class="panel panel-<?= ($update) ? "warning" : "info" ?>">
	        ...
	        <div class="panel-body">
                ...
                <form action="<?= $_SERVER["REQUEST_URI"] ?>" method="post" enctype="multipart/form-data">
                  ...
                  <button type="submit" name="import_csv" class="btn btn-success btn-block">Import CSV</button>
                </form>
